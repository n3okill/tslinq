    ...
    describe("Enumerable", function () {
        test("basic", function () {
            ...
            const grouping = e.groupBy(
                ...
                (pet) => pet.Age,
                (baseAge, ages) => {
                    return { Key: baseAge, Count: ages.count(), Min: ages.min(), Max: ages.max() };
                }
            );
    ...
    describe("EnumerableAsync async", function () {
        test("basic", async function () {
            ...
            const grouping = e.groupBy(
                ...
                async (baseAge, ages) => {
                    return { Key: baseAge, Count: await ages.count(), Min: await ages.min(), Max: await ages.max() };
                }
            );
            expect(await grouping.toArray()).to.be.eql([
                { Count: 1, Key: 8, Max: 8.3, Min: 8.3 },
        ...
        test("OddEven", async function () {
            const groupBy = EnumerableAsync.asEnumerableAsync([1, 2, 3, 4, 5, 6, 7, 8, 9]).groupBy(async (x) => Promise.resolve(x % 2));
            expect(await groupBy.toArray()).to.be.eql([
                [1, 3, 5, 7, 9],
                [2, 4, 6, 8],
            ]);
