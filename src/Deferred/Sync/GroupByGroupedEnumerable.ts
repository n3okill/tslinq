...
class Grouping<TKey, T> extends Enumerable<T> implements Interfaces.IGrouping<TKey, T> {
    constructor(source: Iterable<T>, public Key: TKey) {
        super(source);
    }
}

export class GroupByGroupedEnumerable<T, TKey, TElement> extends Enumerable<Grouping<TKey, TElement>> {
    constructor(first: Enumerable<T>, private readonly _keySelector: Types.TKeySelector<T, TKey>, private readonly _elementSelector: Types.TElementSelector<T, TElement>, private readonly _comparer: Interfaces.IEqualityComparer<TKey>) {
