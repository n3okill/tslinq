...
export class ZipEnumerable<T, TSecond, TThird, TResult> extends Enumerable<TResult> {
    constructor(source: Enumerable<T>, private readonly _second: Iterable<TSecond>, private readonly _third: Iterable<TThird> | undefined, private readonly _resultSelector: CallableFunction) {
    ...
    private *_Zip() {
        ...
        for (let current1 = iterator1.next(), current2 = iterator2.next(); current1.done !== true && current2.done !== true; current1 = iterator1.next(), current2 = iterator2.next()) {
            yield this._resultSelector(current1.value, current2.value);
        }
    }
}
