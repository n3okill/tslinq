...
export class ZipEnumerable<T, TSecond, TThird, TResult> extends EnumerableAsync<TResult> {
  constructor(
    ...
    private readonly _second: AsyncIterable<TSecond>,
    private readonly _third: AsyncIterable<TThird> | undefined,
    private readonly _resultSelector: Types.ZipResultSelectorAsync<T, TSecond, TThird, TResult>,
  ) {
  ...
  private async *_Zip() {
    ...
    const iterator3 = this._third
      ? this._third[Symbol.asyncIterator]()
      : Helpers.asAsyncIterable([])[Symbol.asyncIterator]();
