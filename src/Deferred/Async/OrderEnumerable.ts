...
export class OrderEnumerable<T, TKey> extends EnumerableAsync<T> implements Interfaces.IAsyncOrderedEnumerable<T> {
    constructor(source: EnumerableAsync<T>, private readonly _keySelector: Types.TKeySelectorAsync<T, TKey>, private readonly _comparer: Interfaces.ICompareTo<TKey>, private readonly _isDescending: boolean = false, private readonly _parent?: OrderEnumerable<T, TKey>) {
        super(source);
    }
    ...
    public thenBy<TKey>(keySelector: Types.TKeySelectorAsync<T, TKey>, comparer: Interfaces.ICompareTo<TKey> = new Interfaces.DefaultCompareTo<TKey>()): Interfaces.IAsyncOrderedEnumerable<T> {
        return new OrderEnumerable(this, keySelector, comparer, false, this as unknown as OrderEnumerable<T, TKey>);
    }

    public thenByDescending<TKey>(keySelector: Types.TKeySelectorAsync<T, TKey>, comparer: Interfaces.ICompareTo<TKey> = new Interfaces.DefaultCompareTo<TKey>()): Interfaces.IAsyncOrderedEnumerable<T> {
...
class EnumerableSorter<T, TKey> {
    ...
    constructor(private readonly _keySelector: Types.TKeySelectorAsync<T, TKey>, private readonly _comparer: Interfaces.ICompareTo<TKey>, private readonly _isDescending: boolean = false, private readonly _next?: EnumerableSorter<T, TKey>) {}
    ...
    public CompareKeys(index1: number, index2: number): number {
        const c = this._comparer.CompareTo((this._keys as Array<SorterKey<T, TKey>>)[index1].key, (this._keys as Array<SorterKey<T, TKey>>)[index2].key);
        if (c === 0) {
            if (!this._next) {
