...
export class GroupByEnumerable<T, TKey, TResult, TElement> extends EnumerableAsync<TResult> {
  constructor(
    ...
    private _keySelector: Types.TKeySelectorAsync<T, TKey>,
    private _elementSelector: Types.TElementSelectorAsync<T, TElement>,
    private _resultSelector: Types.TResultSelectorAsync<TKey, TElement, TResult>,
    private _comparer: Types.TIAsyncEqualityComparer<TKey>,
  ) {
  ...
  private async *_GroupBy() {
    ...
    while (current.done !== true) {
      if (isResultSelectorAsync) {
        yield await this._resultSelector(
          current.value[0],
          new EnumerableAsync(Helpers.asAsyncIterable(current.value[1])),
        );
      } else {
