...
export class GroupByGroupedEnumerable<T, TKey, TElement> extends EnumerableAsync<Grouping<TKey, TElement>> {
  constructor(
    ...
    private readonly _elementSelector: Types.TElementSelectorAsync<T, TElement>,
  ...
  public override async toMap<K, V>(): Promise<Map<K, Array<V>>> {
    ...
    for await (const value of this) {
      result.set(
        value.Key as unknown as K,
        (await Helpers.asyncIteratorAsArray(value[Symbol.asyncIterator]())) as unknown as Array<V>,
      );
    }
    return result;
  }
