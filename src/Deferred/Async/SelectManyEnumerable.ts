...
export class SelectManyEnumerable<T, TResult, TCollection> extends EnumerableAsync<TResult> {
  constructor(
    ...
    private readonly _selector: Types.SelectManySelectorAsync<T, TCollection>,
    private readonly _resultSelector: Types.SelectManyResultSelectorAsync<T, TCollection, TResult>,
  ) {
  ...
  private async *_SelectMany() {
    ...
    while (current.done !== true) {
      ...
      const iteratorSelector = isAsyncIterable(selector)
        ? (selector as unknown as AsyncIterable<TCollection>)[Symbol.asyncIterator]()
        : (selector as unknown as Iterable<TCollection>)[Symbol.iterator](); //(await this._selector(current.value, index))[Symbol.asyncIterator]();
