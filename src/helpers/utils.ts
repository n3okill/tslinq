...
export function isAsyncFunction(arg: unknown): arg is CallableFunction {
  ...
  if ("isPromise" in util.types && util.types.isPromise(arg)) {
    return true;
  }
  let promise;
  try {
...
export function isFunctionType(arg: unknown): arg is CallableFunction {
