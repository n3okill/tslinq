...
export type AggregateFunctionType<TSource, TAccumulate> = (
  result: TSource | TAccumulate,
  current: TSource,
) => TAccumulate;
export type AggregateResultType<TAccumulate, TResult> = (result: TAccumulate) => TResult;
export type AggregateFunctionTypeAsync<TSource, TAccumulate> = (
  result: TSource | TAccumulate,
  current: TSource,
...
export type TResultSelectorAsync<TKey, TElement, TResult> = (
  x: TKey,
  y: Interfaces.IAsyncEnumerable<TElement> | Interfaces.IEnumerable<TElement>,
) => TResult | Promise<TResult>;

export type TResultSelectorJoinAsync<TKey, TElement, TResult> = (x: TKey, y: TElement) => TResult | Promise<TResult>;

export type ZipResultSelectorAsync<T, TSecond, TThird, TResult> = (
  x: T,
  y: TSecond,
  z?: TThird,
) => TResult | Promise<TResult>;
