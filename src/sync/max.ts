...
export function max<T>(
  ...
  if (selectorOrComparer instanceof Comparer) {
    if (selectorOrComparer !== Comparer.default) {
      return maxComparer(enumerable, selectorOrComparer);
    }
  ...
  if (typeof key !== "number" || isNaN(key)) {
    throw new InvalidElementsCollection(
      `Items must be of type number in order to use max() - Got '${key}' transformed from '${current.value}'`,
    );
  }
  let result = key;
...
function maxComparer<T>(enumerable: IEnumerable<T>, comparer: Comparer<T>): T {
  const iterator = enumerable.iterator();
  let current = iterator.next();
...
export async function maxAsync<T>(
  ...
  if (
    ...
    if (
      ...
      return maxComparerAsync(enumerable, selectorOrComparer);
    }
  } else if (!isUndefined(selectorOrComparer)) {
    validateArgumentOrThrow(selectorOrComparer, "transformer", "function");
  }
...
async function maxComparerAsync<T>(
  enumerable: IAsyncEnumerable<T>,
